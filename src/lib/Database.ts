...
class DatabaseConstructor {
    ...
    constructor(table: string, identifier?: string) {
        ...
        if(table.includes(':') || id.includes(':')) throw Error(`The database "${table}" table name or identifier cannot include a ":"`);
        this.table = table;
...
const Database = (table: string, identifier?: string) => new DatabaseConstructor(table, identifier);
